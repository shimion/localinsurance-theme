    ...
    class TGM_Plugin_Activation {
        ...
        public function init() {
            ...
            $this->strings = array(
                ...
                'notice_can_install_required'    => _n_noop( 'This theme requires the following plugin (included): %1$s.', 'This theme requires the following plugins (included): %1$s.', 'tgmpa' ),
                'notice_can_install_recommended' => _n_noop( 'This theme recommends the following plugin (included): %1$s.', 'This theme recommends the following plugins (included): %1$s.', 'tgmpa' ),
            ...
            foreach ( $this->plugins as $plugin ) {
                if (
                    ! is_plugin_active( $plugin['file_path'] ) || (
                        isset($installed_plugins) &&
                        version_compare( $installed_plugins[$plugin['file_path']]['Version'], $plugin['version'], '<' ) )
                    ) {
                    add_theme_page(
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                $show_update_link  = $update_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['update_link'], $update_link_count, 'tgmpa' ) . '</a>' : '';
